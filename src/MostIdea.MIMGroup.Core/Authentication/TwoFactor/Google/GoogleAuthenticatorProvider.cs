...
namespace MostIdea.MIMGroup.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : MIMGroupServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("MostIdea.MIMGroup", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
