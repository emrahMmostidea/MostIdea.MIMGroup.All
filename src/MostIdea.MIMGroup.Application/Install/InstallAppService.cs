...
namespace MostIdea.MIMGroup.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : MIMGroupAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<MIMGroupDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{MIMGroupConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{MIMGroupConsts.ConnectionStringName}", constring);
        }
