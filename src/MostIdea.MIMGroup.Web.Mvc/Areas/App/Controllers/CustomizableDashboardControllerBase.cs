...
using MostIdea.MIMGroup.Web.Areas.App.Models.CustomizableDashboard;
...
{
    public abstract class CustomizableDashboardControllerBase : MIMGroupControllerBase
    {
        ...
        protected async Task<ActionResult> GetView(string dashboardName)
        {
            var dashboardDefinition = DashboardCustomizationAppService.GetDashboardDefinition(
                ...
                {
                    DashboardName = dashboardName,
                    Application = MIMGroupDashboardCustomizationConsts.Applications.Mvc
                }
            );
